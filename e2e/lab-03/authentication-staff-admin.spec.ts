...
test.describe("Lab 3 End-to-End Authentication, IT Staff Queue & Admin User Management", () => {
  ...
  test("E2E-LAB3-01: Login & IT Staff Ticket Queue Workflow", async ({ page }) => {
    // 1. Switch to Real Login mode if on dev-selector
    const loginSwitch = page.getByRole("button", { name: /Switch to Real Login/i });
    if (await loginSwitch.isVisible()) {
    ...
    await page.locator("#emailInput").fill("michael@toktickit.com");
    ...
    // 4. Click 'Manage' on the first ticket to open Staff Ticket Detail
    const manageBtn = page.getByRole("button", { name: /Manage/i }).first();
    if (await manageBtn.isVisible()) {
      await manageBtn.click();
      await expect(page.getByRole("heading", { name: /Staff Controls & Workflow/i })).toBeVisible();
  ...
  test("E2E-LAB3-02: Administrator User Management Workflow", async ({ page }) => {
    // 1. Switch to Real Login mode
    const loginSwitch = page.getByRole("button", { name: /Switch to Real Login/i });
    if (await loginSwitch.isVisible()) {
      await loginSwitch.click();
    }

    // 2. Login as Administrator (Admin User)
    await page.locator("#emailInput").fill("admin@toktickit.com");
    await page.locator("#passwordInput").fill("Password123!");
    await page.getByRole("button", { name: /Sign In/i }).click();

    // 3. Verify Admin view navigation
